...
def get_machine_gun_creature_b(position, i):
    return SimulationCreature(
        ...
        name=f"Machine_Gun {i}",
        max_turn_rate=random.randint(5, 25),
        shoot_cooldown=random.randint(2, 8),
        bounding_box_size=(50, 100),
        damage=random.randint(10, 30), # Machine Gun damage range
        bullet_speed=random.randint(20, 50), # Machine Gun bullet speed range
        bullet_range=random.randint(180, 600), # Machine Gun bullet range
    )
...
class AutoChessBatchedSimulator:
    ...
    def run_simulation(self):
        self.game = self.initialize_balanced()
        while True:
...
if __name__ == "__main__":
    num_simulations = 400 # Number of simulations to run
    ...
    simulator = AutoChessBatchedSimulator(arena_sizes, creature_types, 3, 1000) # Initialize the simulator
